...
const fondWidth = window.innerWidth;
const fondHeight = window.innerHeight;
...
wall.setAttribute('width', fondWidth);
wall.setAttribute('height', fondHeight);

class Figure {
  constructor() {
    this.x = Math.floor(Math.random() * fondWidth);
    this.y = Math.floor(Math.random() * fondHeight);
    this.newX = 0;
    this.newY = 0;
    this.size = randomNumber(0.1, 0.6, true);
    this.color = '#ffffff';
    this.strokeWidth = 5 * this.size;
    this.isRandomNewPoint = (amt / 2) < randomNumber(0, amt);
  }
...
function animateFigure() {
  ...
  ctx.clearRect(0, 0, fondWidth, fondHeight);
...
function init() {
  for (let i = 0; i < amt; i++) {
    const figure = (i <= amt / 2) ? new Tic() : new Tac();
